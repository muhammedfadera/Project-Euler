...
to_num <- function(v){
  theList <- list()
  i = 1
  n = 0 ## number of elements to be in dex at a time from v
  counter = 1
  ...
  while(s < length(v)){
    ...
    counter <- counter + 1
    s = s + n ## number of element after each iteration
  }
  return(theList)
}

dtriangle <- to_num(myNum)


summand = c(dtriangle[[1]], numeric(length(dtriangle) - 1))

for(i in 2:length(dtriangle)){
#
